...
do
	...
	if [[ $prompt_input == "exit" ]] ; then
		...
		sudo python3 pymap.py -t $pymap_0x1 > /usr/bin/online_hosts_pymap.txt
		echo "Calculating total amount of online hosts..." | randtype -t 5,12000 | lolcat
		cat /usr/bin/online_hosts_pymap.txt
		cat online_hosts_pymap.txt | grep -v -e "-----------------------------------" -e "IP Address" -e "MAC Address" | wc -l > /usr/bin/total_hosts_online_pymap.txt
		...
		sudo /usr/bin/python3 pyport.py $pyport_0x1
		echo "================================================================" | lolcat
